...
- `mike` — the port owner. Checks `ver` at connect (refuses a body it
  doesn't understand), forwards stdin lines to the body and prints the
  replies, and between commands polls each sensor at its own rate —
  currently `tel` at 1 Hz and `imu` at 5 Hz, one `{cmd, period}` table
  row per sensor. Polling is silent by default; the `-l` flag or the
  stdin command `/log` toggles echoing poll replies to stdout,
  prefixed with the poll command. Scripted seam unchanged:
  `printf 'ping\ntel\n' | ./mike`.
- It also owns the port-handling lore: raw mode, `CLOCAL`, `HUPCL`
  cleared so exiting doesn't drop DTR and reset the body, 0.1 s read
  slices, ~1 s reply timeout, and a 2 s settle plus input flush at
  open to absorb the auto-reset boot chatter.
...
Mike will speak — to humans, when the persona lands; the rover never
performs for itself. One mono voice through the one speaker
